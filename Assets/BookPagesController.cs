...
public class BookPagesController : MonoBehaviour
{
    ...
    [SerializeField] private int objectsPerPage = 2;
    ...
    {
        if (currentPage == book.objects.Count - 1)
        {
            book.Complete();
        }
        else
    ...
    public void UpdateNextAndPreviousButtons ()
    {
        ...
        if (isStickerBook)
            totalPages = Mathf.CeilToInt((float)book.allSubjects.Count / objectsPerPage);
        else
            totalPages = book.objects.Count;
